...
class GenericPumpControl(QtWidgets.QWidget):
    def __init__(self,
        ...
        self.com_port = parameters.get("pump_com_port", "COM7")
        ...
        pump_module = importlib.import_module(
            parameters.get("pump_class", "storm_control.fluidics.pumps.hamilton_psd4"))
...
class SyringePumpControl(GenericPumpControl):
    def __init__(self,
                 parameters = False,
                 parent = None):
        print("** Syringe Pump Control, 2024")
        ...
        self.max_volume = parameters.get("max_volume", 5.0) # mL
        self.min_volume = parameters.get("min_volume", 0.0)
        self.port_names = parameters.get("port_names", "Pull,Exhaust,Bypass").split(',') # corresponds to port 1, 2, 3
    ...
    # ----------------------------------------------------------------------------------------
    def pollPumpStatus(self):
        time.sleep(0.1) # Allow time for pump to respond
        try:
            self.updateStatus(self.pump.getStatus())
        except:
            time.sleep(0.2)
    ...
    def initializePump(self):
        ...
        time.sleep(0.1)
# ----------------------------------------------------------------------------------------
# Peristatic PumpControl Class Definition
# ----------------------------------------------------------------------------------------
